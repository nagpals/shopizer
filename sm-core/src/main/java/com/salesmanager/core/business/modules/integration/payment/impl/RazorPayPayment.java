...
//	@Value("${PAYMENT.RAZORPAY.KEY}") // TODO Hemant , take this from the database
//	private String razorpayKey;

//	@Value("${PAYMENT.RAZORPAY.SECRET}") // TODO Hemant , take this from the database
//	private String razorpaySecret;
	...
	public Transaction capture(MerchantStore store, Customer customer, Order order, Transaction capturableTransaction,
		...
		try {
			 ...
			 * &trnType=PAC&username=user1234&password=pass1234&trnID=1000 2115 --> requires
			 * also adjId [not documented]
			 **/
			...
			LOGGER.debug("REQUEST SENT TO Razorpay -> " + messageString.toString());

			return sendTransaction(null, store, messageString.toString(), "PAC", TransactionType.CAPTURE,
			...
			throw new IntegrationException("Error while processing Razorpay transaction", e);
	...
	public Transaction authorizeAndCapture(MerchantStore store, Customer customer, List<ShoppingCartItem> items,
			BigDecimal amount, Payment payment, IntegrationConfiguration configuration, IntegrationModule module)
			throws IntegrationException {
		return processTransaction(store, customer, TransactionType.AUTHORIZECAPTURE, amount, payment, configuration,
				module); // hemant , api is calling this 
	}
	...
	public Transaction refund(boolean partial, MerchantStore store, Transaction transaction, Order order,
		...
		try {
			...
			String server = new StringBuffer().append(configs.getScheme()).append("://").append(configs.getHost())
					.append(":").append(configs.getPort()).append(configs.getUri()).toString();
			...
			return sendTransaction(null, store, messageString.toString(), "R", TransactionType.REFUND,
	...
	private Transaction sendTransaction(String orderNumber, MerchantStore store, String transaction,
			String beanstreamType, TransactionType transactionType, PaymentType paymentType, BigDecimal amount,
			IntegrationConfiguration configuration, IntegrationModule module) throws IntegrationException {
		...
		try {
			 ...
			 * &orderNumber=caa71106-7e3f-4975-a657-a35904dc32a0 &trnCardOwner=Carl Samson
			 * &trnCardNumber=5100000020002000 &trnExpMonth=10 &trnExpYear=14
			 * &trnCardCvd=123 &trnAmount=77.01 &ordName=Carl S &ordAddress1=378 Du
			 * Languedoc &ordCity=Boucherville &ordProvince=QC &ordPostalCode=J3B8J1
			 * &ordCountry=CA &ordPhoneNumber=(444) 555-6666 &ordEmailAddress=test@yahoo.com
			 **/
			...
			server = new StringBuffer().append(configs.getScheme()).append("://").append(configs.getHost()).append(":")
					.append(configs.getPort()).append(configs.getUri()).toString();
			...
			if (rc != -1) {
				...
				LOGGER.debug("Razorpay response -> " + respText.trim());

				nvp = formatUrlResponse(respText.trim());
			} else {
				throw new IntegrationException("Invalid response from Razorpay, return code is " + rc);
			}
			...
			if (StringUtils.isBlank(transactionApproved)) {
				throw new IntegrationException("Required field transactionApproved missing from Razorpay response");
			}
			...
			if (transactionApproved.equals("0")) {
				...
				merchantLogService.save(new MerchantLog(store,
						"Can't process Razorpay message " + messageText + " return code id " + messageId));

				IntegrationException te = new IntegrationException("Can't process Razorpay message " + messageText);
	...
	private Transaction processTransaction(MerchantStore store, Customer customer, TransactionType type,
		...
		try {
			...
			// TODO do the transaction here
			
			Map<String, String> nvp = new HashMap<>();
			//TODO pur various fields which are required for further processing into nvp
			
			return parseResponse(TransactionType.AUTHORIZECAPTURE, PaymentType.CREDITCARD, nvp, amount);
			
			
//			CreditCardPayment creditCardPayment = (CreditCardPayment) payment;  // TODO Hemant , change from here
//
//			messageString.append("requestType=BACKEND&");
//			messageString.append("merchant_id=").append(configuration.getIntegrationKeys().get("merchantid"))
//					.append("&");
...
//			 * 
//			 * Prior to processing a pre-authorization through the API, you must modify the
//			 * transaction settings in your Razorpay merchant member area to allow for this
//			 * transaction type. - Log in to the Beanstream online member area at
//			 * www.beanstream.com/admin/sDefault.asp. - Navigate to administration - account
//			 * admin - order settings in the left menu. Under the heading �Restrict Internet
//			 * Transaction Processing Types,� select either of the last two options. The
//			 * �Purchases or Pre-Authorization Only� option will allow you to process both
//			 * types of transaction through your web interface. De-selecting the �Restrict
//			 * Internet Transaction Processing Types� checkbox will allow you to process all
//			 * types of transactions including returns, voids and pre-auth completions.
//			 * 
...
//			 * 
//			 * 
//			 * //notes //On receipt of the transaction response, the merchant must display
//			 * order amount, transaction ID number, bank authorization code (authCode),
//			 * currency, date and �messageText� to the customer on a confirmation page.
//			 */
//
//			// String agent = "Mozilla/4.0";
//			// String respText = "";
...
//			/** debug **/
//
//			LOGGER.debug("REQUEST SENT TO Razorpay -> " + messageLogString.toString());
//
//			URL postURL = new URL(server);
//			conn = (HttpURLConnection) postURL.openConnection();

//			return sendTransaction(orderNumber, store, messageString.toString(), transactionType, type,
//					payment.getPaymentType(), amount, configuration, module);
	...
	private Transaction parseResponse(TransactionType transactionType, PaymentType paymentType, Map<String, String> nvp,
	...
	public void validateModuleConfiguration(IntegrationConfiguration integrationConfiguration, MerchantStore store)
		...
		// TODO Hemant : Add integration keys requirement on to Razorpay.jsp. and then
		// admin will add this to make payments.app
...
//		if(keys==null || StringUtils.isBlank(keys.get("merchantid"))) {  // TODO Hemant changed it 
...
//		if(keys==null || StringUtils.isBlank(keys.get("username"))) { // TODO Hemant changed it
...
//		if(keys==null || StringUtils.isBlank(keys.get("password"))) { // TODO Hemant commented it as it is not required for Razropay
//			if(errorFields==null) {
//				errorFields = new ArrayList<String>();
//			}
