...
public class S3ProductContentFileManager implements ProductAssetsManager {
	...
	public void removeProductImage(ProductImage productImage) throws ServiceException {
		try {
			...
			s3.deleteObject(bucketName,
					nodePath(productImage.getProduct().getMerchantStore().getCode(), productImage.getProduct().getSku())
	...
	public void addProductImage(ProductImage productImage, ImageContentFile contentImage) throws ServiceException {
		...
		try {
			...
			PutObjectRequest request = new PutObjectRequest(bucketName, nodePath + productImage.getProductImage(),
					contentImage.getFile(), metadata);
			request.setCannedAcl(CannedAccessControlList.PublicRead);

			System.out.println(request.getRequestCredentialsProvider());
	...
	private AmazonS3 s3Client() {
		...
		LOGGER.debug("AWS Product  Using region " + region);
		...
		AmazonS3 amazonS3Client = AmazonS3ClientBuilder.standard()
				...
				withRegion(region)
				.build();
//		return AmazonS3ClientBuilder.standard().withRegion(regionName()).build(); // The first region to try your
	...
	private String nodePath(String store) {
		return new StringBuilder().append(ROOT_NAME).append(Constants.SLASH).append(store).append(Constants.SLASH)
				.toString();
	}
